  ...
  return (
    <>
      {posts.map((post) => (
        <div
          key={post._id}
          className="p-4 mx-4 mt-4 border border-muted-foreground rounded-md flex flex-row"
          onClick={() => {
            setActivePost(post);
            map?.panTo([post.latitude, post.longitude]);
          }}
        >
          {post.imageUrl && (<div className=""><img 
            className="rounded mr-3 object-cover"
            src={`${import.meta.env.VITE_API_BASE}/images/posts/${post.imageUrl}`}
            width={60}
            height={60}
          /></div>)}
          <div>
